...
// Override the generateClientRows function to include both storage locations
window.generateClientRows = function() {
    // Get clients from both storage locations
    ...
    const allClients = [...insuranceClients];
    ...
    console.log('Total clients after merge:', allClients.length);
...
// Also update the loadClientsView to use merged data for the count
const originalLoadClientsView = window.loadClientsView;
window.loadClientsView = function() {
    // Call original function
    if (originalLoadClientsView) {
        originalLoadClientsView();
    }
    
    // Update count with merged data
    setTimeout(() => {
        const insuranceClients = JSON.parse(localStorage.getItem('insurance_clients') || '[]');
        const convertedClients = JSON.parse(localStorage.getItem('clients') || '[]');
